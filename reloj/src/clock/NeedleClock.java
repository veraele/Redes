               ...
               ImageObserver
{
  ...
  public void run ()
  {
    ...
    while (timer == thread)
    {
      ...
      if (oldSeconds != seconds) 
      {
          ...
          needleSec.setAngle ((seconds%60) * (2*Math.PI/360)-(2*Math.PI/3));
  ...
  void drawDivisions (Graphics g)
  {
    ...
    int p;
    for (angle = 0d,p=0; angle < 2*Math.PI; angle += SIXTY,p++)
    {
    	if(!(p>4&&p<25))
    	{
    ...
    for (angle=0d,p=0; angle < 2*Math.PI; angle += TWELVE,p++)
    {
    	if(!(p>1&&p<5))
    	{
  ...
  void drawNumbers (Graphics g)
  {
    ...
    int radius = this.radius - height*2;
